...
import sys
import os
from converters.geojson_to_svg import geojson_to_svg  # Import your SVG function
...
from PySide6.QtWidgets import (
    ...
    QWidget, QMessageBox, QToolBar, QStackedWidget, QListWidget, QGridLayout
)
...
class MainWindow(QMainWindow):
    ...
    def create_main_context(self):
        ...
        # Add layers for Lots, Grass, Water, Road
        self.create_file_input(input_grid, "Lots", 0)
        self.create_file_input(input_grid, "Grass", 1)
        self.create_file_input(input_grid, "Water", 2)
        self.create_file_input(input_grid, "Road", 3)

        layout.addLayout(input_grid)

        # File list for all selected layers
        ...
        for layer, widget in self.file_list_widgets.items():
            layout.addWidget(QLabel(f"{layer} Files:"))
    ...
    def remove_files(self, file_type):
        """Remove selected files from the given file type."""
    ...
    def update_file_list(self, file_type):
        """Update the file list widget for the given file type."""
        widget = self.file_list_widgets[file_type]
        widget.clear()
        widget.addItems(self.file_paths[file_type])
